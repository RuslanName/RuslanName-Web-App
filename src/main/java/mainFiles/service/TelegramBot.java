...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private void addingProduct(Message message) throws TelegramApiException, MalformedURLException {
        ...
        if (differentState.getProductName() == null) {
            ...
            addProductDB(message, differentState.getProductName(), differentState.getProductQuantity(),
                    differentState.getProductPrice());
        }
    }
    ...
    private void sendAdminPanel(long chatId, String action) {
        ...
        if (action.equals("Старт")) {
            message.setText("Добро пожаловать, администратор " + user.getFirstName());
        }
    ...
    private void addProductDB(Message message, String name, int quantity, int price) throws TelegramApiException, MalformedURLException {
        long chatId = message.getChatId();
    ...
    private void deleteProductDB(Long chatId, int id) {
        var products = productsRepository.findAll();
        String iconPathToDelete = null;

        for (Product product : products) {
            if (product.getId() == id) {
                iconPathToDelete = product.getIconPath();
                productsRepository.deleteById(id);
                break;
            }
        }

        if (iconPathToDelete != null) {
            try {
                Files.deleteIfExists(Paths.get(iconPathToDelete));
            } catch (IOException e) {
    ...
    private String saveProductIcon(Message message) throws TelegramApiException, MalformedURLException {
        ...
        try (InputStream in = new URL(fileUrl).openStream()) {
            Files.copy(in, Paths.get(iconPath));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        return iconFileName;
    ...
    private void sendProductIcon(long chatId, String iconPath) {
